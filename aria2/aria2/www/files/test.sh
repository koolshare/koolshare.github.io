...
ip_rule_exist=`ip rule show | grep "fwmark 0x1/0x1 lookup 100" | grep -c 100`
...
sed -e "s/^/-A chnroute &/g" -e "1 i\-N chnroute nethash --hashsize 60000" $chnroute | awk '{print $0} END{print "COMMIT"}' | ipset -R
...
if [ "$ss_lan_control" == "1" ];then
	...
	if [ ! -z $ss_white_lan ];then
		...
		do
			...
			iptables -t mangle -A SHADOWSOCKS2 -p udp -s $white_ip -m set ! --match-set chnroute dst -j TPROXY --on-port 3333 --tproxy-mark 0x01/0x01
		done
	else
	iptables -t nat -A SHADOWSOCKS -p tcp -m set ! --match-set chnroute dst -j REDIRECT --to-ports 3333
	iptables -t mangle -A SHADOWSOCKS2 -p udp -m set ! --match-set chnroute dst -j TPROXY --on-port 3333 --tproxy-mark 0x01/0x01
...
if [ "$ss_chromecast" == "1" ]; then
	...
	echo disable lan dns take over..
	iptables -t nat -D PREROUTING -s $lan_ipaddr/24 -p udp --dport 53 -j DNAT --to $lan_ipaddr >/dev/null 2>&1
	echo $(date): done
	echo $(date):
fi
