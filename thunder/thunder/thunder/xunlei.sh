...
LOG_FILE="xunlei.log"																						#日志文件名称，可以自定义
LOG_DIR="/tmp"																				#日志保存路径，可以自定义
...
if [ -f "/koolshare/thunder/wan-start" ]; then
	CWS=`cat /koolshare/thunder/wan-start|grep "${CWS_X}"`
	if [ -z "${CWS}" ]; then
		echo "$(date +%Y年%m月%d日\ %X)： 调整自启动方案，启用多线程并发自启动方案……"
		sed -i "1a ${CWS_X}" "/koolshare/thunder/wan-start"
	else
		echo "$(date +%Y年%m月%d日\ %X)： 清除可能引起冲突的自启动命令……"
		sed -i "/$(basename $0)/d" "/koolshare/thunder/wan-start"
		echo "$(date +%Y年%m月%d日\ %X)： 启用用多线程并发自启动方案……"
		sed -i "1a ${CWS_X}" "/koolshare/thunder/wan-start"	
	fi
else
	cat > "/koolshare/thunder/wan-start" <<EOF
#!/bin/sh
${CWS_X}
EOF
fi
chmod 755 "/koolshare/thunder/wan-start"
if [ -z "$(dbus list __|grep "/koolshare/thunder/wan-start")" ]; then
		echo "$(date +%Y年%m月%d日\ %X)： 将多线程并发自启动脚本添加到系统自启动……"
		dbus event onwanstart_wan-start "/koolshare/thunder/wan-start"
fi
}
...
wget -c -N -q --tries=3 --timeout=5 -O getsysinfo.txt http://127.0.0.1:9000/getsysinfo
		if [ -e "getsysinfo.txt" ]; then
				...
				VERSION=`cut -d '"' -f4 getsysinfo.txt`
...
while true; do
		...
		PID_xware_guard=\`ps|grep -E "sh\(.*\)$(basename $0)|sleep 10m"|grep -v grep|awk '{print \$1}'\`
