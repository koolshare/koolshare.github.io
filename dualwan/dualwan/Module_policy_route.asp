...
conf2obj();
version_show();
write_dualwanpolicy_install_status();
...
function version_show(){
	if (db_dualwanpolicy_['dualwanpolicy_version_local'] != db_dualwanpolicy_['dualwanpolicy_version_web'] && db_dualwanpolicy_['dualwanpolicy_version_web'] !== "undefined"){
		...
		$("#updateBtn").html("<i>升级到：" + db_dualwanpolicy_['dualwanpolicy_version_web']);
	} else {
		$("#dualwan_version_show").html("<i>当前版本：" + db_dualwanpolicy_['dualwanpolicy_version_local']);
	}
}

function write_dualwanpolicy_install_status(){
	$.ajax({
		type: "get",
		url: "dbconf?p=dualwanpolicy_",
		dataType: "script",
		success: function() {
		if (db_dualwanpolicy_['dualwanpolicy_install_status'] == "1"){
			$("#dualwanpolicy_install_show").html("<i>正在下载更新...</i>");
		} else if (db_dualwanpolicy_['dualwanpolicy_install_status'] == "2"){
			$("#dualwanpolicy_install_show").html("<i>正在安装更新...</i>");
		} else if (db_dualwanpolicy_['dualwanpolicy_install_status'] == "3"){
			$("#dualwanpolicy_install_show").html("<i>安装更新成功，5秒后刷新本页,你需要重新开启dualwanpolicy</i>");
			version_show();
			refreshpage(3);
		} else if (db_dualwanpolicy_['dualwanpolicy_install_status'] == "4"){
		   document.getElementById('updateBtn').style.display = "";
			$("#dualwanpolicy_install_show").html("<i>下载文件校验不一致！</i>");
		} else if (db_dualwanpolicy_['dualwanpolicy_install_status'] == "5"){
			document.getElementById('updateBtn').style.display = "";
			$("#dualwanpolicy_install_show").html("<i>然而并没有更新！</i>");
		} else if (db_dualwanpolicy_['dualwanpolicy_install_status'] == "6"){
      $("#dualwanpolicy_install_show").html("<i>正在检查是否有更新~</i>");
		} else if (db_dualwanpolicy_['dualwanpolicy_install_status'] == "7"){
		   document.getElementById('updateBtn').style.display = "";
			$("#dualwanpolicy_install_show").html("<i>检测更新错误！</i>");
		} else {
			$("#dualwanpolicy_install_show").html("");
		}
		setTimeout("write_dualwanpolicy_install_status()", 1000);
		}
		});
}

function update_dualwanpolicy(o, s){
	document.form.dualwanpolicy_update_check.value = 1;
...
<button id="updateBtn" class="button_gen" onclick="update_dualwanpolicy(this, ' Refresh ');">检查更新</button>
<a style="margin-left: 178px;" href="https://github.com/koolshare/koolshare.github.io/blob/master/dualwan/Changelog.txt" target="_blank"><em>[<u> 更新日志 </u>]</em></a>
</div>
...
<option value="5">中国ip段</option>
<option value="6">自定义配置文件</option>
</select>
