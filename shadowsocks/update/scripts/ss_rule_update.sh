...
echo =======================================================================================================
echo $(date): 开始更新shadowsocks规则，请等待...
...
if [ "$?" == "0" ]; then
	echo $(date): 检测到在线版本文件，继续...
else
	echo $(date): 没有检测到在线版本欸，可能是访问github有问题，去大陆白名单模式试试吧！
	rm -rf /tmp/version1
	exit
fi
...
if [ "$ss_basic_gfwlist_update" == "1" ];then
	if [ ! -z "$version_gfwlist2" ];then
		if [ "$version_gfwlist1" != "$version_gfwlist2" ];then
			echo $(date): 检测到新版本gfwlist，开始更新...
			echo $(date): 下载gfwlist到临时文件...
			...
			if [ "$md5sum_gfwlist1"x = "$md5sum_gfwlist2"x ];then
				echo $(date): 下载完成，校验通过，将临时文件覆盖到原始gfwlist文件
				mv /tmp/gfwlist.conf /koolshare/ss/ipset/gfwlist.conf
				sed -i "1s/.*/$git_line1/" /koolshare/ss/cru/version
				reboot="1"
				echo $(date): 你的gfwlist已经更新到最新了哦~
			else
				echo $(date): 下载完成，但是校验没有通过！
			fi
		else
			echo $(date): 检测到gfwlist本地版本号和在线版本号相同，那还更新个毛啊!
		fi
	else
		echo $(date): gfwlist文件下载失败！
	fi
else
	echo $(date): 然而你并没有勾选gfwlist更新！
fi
...
if [ "$ss_basic_chnroute_update" == "1" ];then
	if [ ! -z "$version_chnroute2" ];then
		if [ "$version_chnroute1" != "$version_chnroute2" ];then
			echo $(date): 检测到新版本chnroute，开始更新...
			echo $(date): 下载chnroute到临时文件...
			...
			if [ "$md5sum_chnroute1"x = "$md5sum_chnroute2"x ];then
				echo $(date): 下载完成，校验通过，将临时文件覆盖到原始chnroute文件
				mv /tmp/chnroute.txt /koolshare/ss/redchn/chnroute.txt
				sed -i "2s/.*/$git_line2/" /koolshare/ss/cru/version
				reboot="1"
				echo $(date): 你的chnroute已经更新到最新了哦~
			else
				echo $(date): md5sum 下载完成，但是校验没有通过！
			fi
		else
			echo $(date): 检测到chnroute本地版本号和在线版本号相同，那还更新个毛啊!
		fi
	else
		echo $(date): file chnroute文件下载失败！
	fi
else
	echo $(date): 然而你并没有勾选chnroute更新！
fi
...
if [ "$ss_basic_cdn_update" == "1" ];then
	if [ ! -z "$version_cdn2" ];then
		if [ "$version_cdn1" != "$version_cdn2" ];then
			echo $(date): 检测到新版本cdn名单，开始更新...
			echo $(date): 下载cdn名单到临时文件...
			...
			if [ "$md5sum_cdn1"x = "$md5sum_cdn2"x ];then
				echo $(date): 下载完成，校验通过，将临时文件覆盖到原始cdn名单文件
				mv /tmp/cdn.txt /koolshare/ss/redchn/cdn.txt
				sed -i "4s/.*/$git_line4/" /koolshare/ss/cru/version
				reboot="1"
				echo $(date): 你的cdn名单已经更新到最新了哦~
			else
				echo $(date): 下载完成，但是校验没有通过！
			fi
		else
			echo $(date): 检测到cdn名单本地版本号和在线版本号相同，那还更新个毛啊!
		fi
	else
		echo $(date): file cdn名单文件下载失败！
	fi
else
	echo $(date): 然而你并没有勾选cdn名单更新！
fi
...
# reboot ss
if [ "$reboot" == "1" ];then
echo $(date): 自动重启shadowsocks，以应用新的规则文件！请稍后！
