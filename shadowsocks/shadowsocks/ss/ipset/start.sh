...
custom_dnsmasq(){
	...
	if [ ! -z "$ss_ipset_dnsmasq" ];then
		echo_date 添加自定义dnsmasq设置到/jffs/configs/dnsmasq.conf.add
...
append_white_black_conf(){
	...
	if [ ! -z $ss_ipset_white_domain_web ];then
		echo_date 创建域名白名单到/tmp/wblist.conf
	...
	if [ ! -z "$ss_ipset_black_domain_web" ];then
		echo_date 创建域名黑名单到/tmp/wblist.conf
...
ln_custom_conf(){
	...
	echo_date 创建自定义dnsmasq配置链接到/jffs/configs/dnsmasq.d/custom.conf
...
start_dns(){
	...
	if [ "$ss_ipset_foreign_dns" == "0" ]; then
		echo_date 开启 dnscrypt-proxy，你选择了"$ss_ipset_opendns"节点.
	...
	if [ "$ss_ipset_foreign_dns" == "4" ]; then
		...
		if [ "$ss_ipset_pdnsd_method" == "1" ];then
			...
			if [ "$ss_ipset_pdnsd_udp_server" == "1" ];then
				...
				echo_date 开启dnscrypt-proxy作为pdnsd的上游服务器,你选择了"$ss_ipset_pdnsd_udp_server_dnscrypt"节点.
			...
			echo_date 创建pdnsd配置文件到/koolshare/ss/pdnsd/pdnsd.conf
			echo_date 你选择了-仅tcp查询-，使用"$ss_ipset_pdnsd_server_ip":"$ss_ipset_pdnsd_server_port"进行tcp查询.
			cat > /koolshare/ss/pdnsd/pdnsd.conf <<-EOF
				global {
					perm_cache=2048;
					cache_dir="/koolshare/ss/pdnsd/";
...
start_all)
	...
	echo_date ------------------- 梅林固件 shadowsocks gfwlist模式 ---------------------
	resolv_server_ip
	creat_ss_json
	creat_dnsmasq_basic_conf
	append_gfwlist
	...
	nvram set ss_mode=1
	nvram commit
	echo_date --------------------- shadowsocks gfwlist模式启动完毕 --------------------
	;;
restart_dns)
	...
	restart_dnsmasq
	remove_status
	echo_date ----------------------- gflist模式-dns服务重启完毕 ------------------------
	;;
restart_wb_list)
	#ss_basic_action=3 应用黑白名单设置
	echo_date ----------------------- gflist模式-重启黑白名单服务 ------------------------
