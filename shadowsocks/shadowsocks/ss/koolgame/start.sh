...
start_all)
	...
	echo_date -------------------- 梅林固件 shadowsocks 游戏模式V2 ----------------------
	detect_qos
	resolv_server_ip
	creat_ss_json
	creat_dnsmasq_basic_conf
	...
	nvram set ss_mode=4
	nvram commit
	echo_date --------------------- shadowsocks 游戏模式V2启动完毕 ----------------------
	;;
restart_dns)
	#ss_basic_action=2 应用DNS设置
	echo_date ------------------------ 游戏模式V2-重启dns服务 ---------------------------
	detect_qos
	stop_dns
	creat_dnsmasq_basic_conf
	#user_cdn_site
	...
	restart_dnsmasq
	remove_status
	echo_date ----------------------- 游戏模式V2-dns服务重启完毕 -------------------------
	;;
restart_addon)
	#ss_basic_action=4 应用附加设置
	echo_date ------------------------ 游戏模式V2-重启附加功能 ---------------------------
	detect_qos
