...
<script type="text/javascript" src="/ss_conf?f=game&v=<% uptime(); %>"></script>
...
var el = document.getElementById("game_"+field);
if(el != null) {
el.value = game[field];
}
var temp_ss = ["game_host"];
...
document.form.action = "/applyss.cgi?f=game";
...
gdc = document.form.game_dns_china.value;
gdf = document.form.game_dns_foreign.value;
gs = document.form.game_sstunnel.value
gcc = document.form.game_chinadns_china.value
gcf = document.form.game_chinadns_foreign.value
showhide("show_isp_dns", (gdc == "1"));
showhide("game_dns_china_user", (gdc == "5"));
showhide("game_dns_china_user_txt1", (gdc !== "5"));
showhide("game_dns_china_user_txt2", (gdc == "5"));
showhide("game_opendns", (gdf == "1"));
showhide("game_sstunnel", (gdf == "2"));
showhide("chinadns_china", (gdf == "3"));
showhide("chinadns_foreign", (gdf == "3"));
showhide("game_sstunnel_user", ((gdf == "2") && (gs == "4")));
showhide("dns_plan_foreign1", (gdf == "1"));
showhide("dns_plan_foreign2", ((gdf == "2") && (gs !== "4")));
showhide("dns_plan_foreign3", ((gdf == "2") && (gs == "4")));
showhide("game_chinadns_china_user", (gcc == "4"));
showhide("chinadns_china1", (gcc !== "4"));
showhide("chinadns_china2", (gcc == "4"));
showhide("game_chinadns_foreign_user", (gcf == "4"));
showhide("chinadns_foreign1", (gcf !== "4"));
showhide("chinadns_foreign2", (gcf == "4"));
showhide("game_dns2socks_user", (gdf == "4"));
						...
						<table width="760px" border="0" cellpadding="5" cellspacing="0" bordercolor="#6b8fa3" class="FormTitle" id="FormTitle">
							<tr>
								<td bgcolor="#4D595D" colspan="3" valign="top">
									...
									<div style="margin-left:5px;margin-top:10px;margin-bottom:10px"><img src="/images/New_ui/export/line_export.png"></div>
										<table id="game_table"width="100%" border="1" align="center" cellpadding="4" cellspacing="0" bordercolor="#6b8fa3" class="FormTable">
											<thead>
											<tr>
											...
											<th width="20%">选择国内DNS</th>
												<td>
													<select id="game_dns_china" name="game_dns_china" class="input_option" onclick="update_visibility();" >
													...
													<input type="text" class="ssconfig input_ss_table" id="game_dns_china_user" name="game_dns_china_user" maxlength="100" value=""></input>
													<span id="show_isp_dns">【<% nvram_get("wan0_dns"); %>】</span> <br/>
													<span id="game_dns_china_user_txt1">默认：运营商DNS【用于解析国内6000+个域名】</span> 
													<span id="game_dns_china_user_txt2">确保你自定义输入的国内DNS在chnroute中</span>
												</td>
											</tr>
											<tr id="dns_plan_foreign">
											<th width="20%">选择国外DNS</th>
												<td>
													<select id="game_dns_foreign" name="game_dns_foreign" class="input_option" onclick="update_visibility();" >
													...
													<select id="game_opendns" name="game_opendns" class="input_option">
													...
													<input type="text" class="ssconfig input_ss_table" id="game_dns2socks_user" name="game_dns2socks_user" maxlength="100" placeholder="需端口号如：8.8.8.8:53" value="">
													<select id="game_sstunnel" name="game_sstunnel" class="input_option" onclick="update_visibility();" >
													...
													<input type="text" class="ssconfig input_ss_table" id="game_sstunnel_user" name="game_sstunnel_user" maxlength="100" placeholder="需端口号如：8.8.8.8:53" value="">
													<br/>
											...
											<th width="20%">ChinaDNS国内DNS</th>
												<td>
													<select id="game_chinadns_china" name="game_chinadns_china" class="input_option" onclick="update_visibility();" >
														<option value="1">阿里DNS1【223.5.5.5】</option>
														<option value="2">阿里DNS2【223.6.6.6】</option>
														<option value="3">114DNS【114.114.114.114】</option>
														<option value="4">自定义</option>
													</select>
													<input type="text" class="ssconfig input_ss_table" id="game_chinadns_china_user" name="game_chinadns_china_user" placeholder="需端口号如：8.8.8.8:53" maxlength="100" value="">
													<br/>
											...
											<th width="20%">ChinaDNS国外DNS</th>
												<td>
													<select id="game_chinadns_foreign" name="game_chinadns_foreign" class="input_option" onclick="update_visibility();" >
														<option value="1">OpenDNS [208.67.220.220]</option>
														<option value="2">Google DNS1 [8.8.8.8]</option>
														<option value="3">Google DNS2 [8.8.4.4]</option>
														<option value="4">自定义</option>
													</select>
													<input type="text" class="ssconfig input_ss_table" id="game_chinadns_foreign_user" name="game_chinadns_foreign_user" maxlength="100" placeholder="需端口号如：8.8.8.8:53" value="">
											...
											<th width="20%">自定义hosts</th>
												<td>
													<textarea cols="50" rows="16" id="game_host" name="game_host" style="width:99%; font-family:'Courier New', Courier, mono; font-size:11px;background:#475A5F;color:#FFFFFF;">
													</textarea>
												</td>
											</tr>
										</table>
									...
									<div id="NoteBox" style="display:none">
										...
										<p> <i>● </i>虽然此处提供了自定义host功能，但是不是很建议建议你将你从网上得到的游戏加速host填入其中，网上爱好者整理的host列表是针对国内直连下载加速的，不过如果你对你的ss服务器有自信，那么你也可以设置，因为这些host地址都是国外地址，用ss下载应该也能达到加速的目的。</p>
										...
										<h3> 自定义hosts：</h3>
										<p>此处你将能够添加你自定义的host，格式参照已经添加的例子。 </p>
									</div>
									<div style="margin-left:5px;margin-top:10px;margin-bottom:10px"><img src="/images/New_ui/export/line_export.png"></div>
